...
function forward_kinematics_absolute(theta1, theta2, l1, l2)
    x1 = l1 * cos(theta1)
    y1 = l1 * sin(theta1)
    x2 = x1 + l2 * cos(theta2)
    y2 = y1 + l2 * sin(theta2)
...
# Animation function
function animate_robots(theta1_traj, theta2_traj, theta3_traj, theta4_traj, d, l1=1.0, l2=1.0, width=0.1, jaw_length=0.2, jaw_width=0.05)
    ...
    @gif for i in 1:n_frames
        ...
        x1, y1, x2, y2 = forward_kinematics_absolute(theta1, theta2, l1, l2)
        
        # Forward kinematics for robot 2 with absolute angles (base shifted by d in y direction)
        x3, y3, x4, y4 = forward_kinematics_absolute(theta3, theta4, l1, l2)
        y3 += d
        y4 += d
